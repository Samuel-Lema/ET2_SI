...
consumed(2022,4,17,22,2,7,beer).
consumed(2022,4,17,22,1,57,beer).
consumed(2022,4,17,22,1,49,beer).
...
@p__17[source(self),url("file:src/asl/myRobot.asl")] +!doSomething(Answer,Ag) : service(Answer,Service) <- .println("Aqui debe ir el código del servicio:",Service," para el agente ",Ag).
...
@p__19[source(self),url("file:src/asl/myRobot.asl")] +!bring(myOwner,beer)[source(myOwner)] <- +asked(beer); !save.
...
@p__21[source(self),url("file:src/asl/myRobot.asl")] +!bringBeer : (asked(beer) & not (healthMsg(_23))) <- .println("Owner me ha pedido una cerveza."); !go_at(myRobot,fridge); !take(fridge,beer); !go_at(myRobot,myOwner); !hasBeer(myOwner); .println("Ya he servido la cerveza y elimino la petición."); .abolish(asked(Beer)); !bringBeer.
@p__22[source(self),url("file:src/asl/myRobot.asl")] +!bringBeer : (not (asked(beer)) & not (healthMsg(_24))) <- .wait(2000); .println("Robot esperando la petición de Owner."); !bringBeer.
@p__23[source(self),url("file:src/asl/myRobot.asl")] +!take(fridge,beer) : not (too_much(beer)) <- .println("El robot está cogiendo una cerveza."); !check(fridge,beer).
...
@p__25[source(self),url("file:src/asl/myRobot.asl")] +!check(fridge,beer) : (not (ordered(beer)) & available(beer,fridge)) <- .println("El robot está en el frigorífico y coge una cerveza."); .wait(1000); open(fridge); .println("El robot abre la nevera."); get(beer); .println("El robot coge una cerveza."); close(fridge); .println("El robot cierra la nevera.").
@p__26[source(self),url("file:src/asl/myRobot.asl")] +!check(fridge,beer) : (not (ordered(beer)) & not (available(beer,fridge))) <- .println("El robot está en el frigorífico y hace un pedido de cerveza."); !comprobarStock; !check(fridge,beer).
@p__27[source(self),url("file:src/asl/myRobot.asl")] +!check(fridge,beer) <- .println("El robot está esperando ................"); .wait(5000); !check(fridge,beer).
...
@p__31[source(self),url("file:src/asl/myRobot.asl")] +!orderBeer(Supermarket) : (not (ordered(beer)) & (dinero(D) & hayStock(P)[source(Supermarket)])) <- .if_then_else((Supermarket == sinStock),.println("Ning�n super tiene stock. Esperando..."); .wait(5000); !comprobarStock,.if_then_else((D < P),.println("No tengo dinero para comprar en ",Supermarket); .wait(5000),.println("El robot realiza un pedido a ",Supermarket); !go_at(myRobot,delivery); .println("El robot va a la ZONA de ENTREGA."); .send(Supermarket,achieve,order(beer,3)); +ordered(beer))); !save.
@p__32[source(self),url("file:src/asl/myRobot.asl")] +!orderBeer(Supermarket).
@p__33[source(self),url("file:src/asl/myRobot.asl")] +!hasBeer(myOwner) : not (too_much(beer)) <- hand_in(beer); .println("He preguntado si Owner ha cogido la cerveza."); ?has(myOwner,beer); .println("Se que Owner tiene la cerveza."); .date(YY,MM,DD); .time(HH,NN,SS); +consumed(YY,MM,DD,HH,NN,SS,beer); !save.
...
@p__38[source(self),url("file:src/asl/myRobot.asl")] +stock(beer,0) : available(beer,fridge) <- -available(beer,fridge); !save.
@p__39[source(self),url("file:src/asl/myRobot.asl")] +stock(beer,N) : ((N > 0) & not (available(beer,fridge))) <- -+available(beer,fridge); !save.
...
@p__42[source(self),url("file:src/asl/myRobot.asl")] +!recibirDinero(X) : dinero(D) <- .println("Recibidos ",X," c�ntimos del owner."); -+dinero((D+X)); !save.
